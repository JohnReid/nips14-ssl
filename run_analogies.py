...
if dataset == 'svhn':
    ...
    train_x, train_y, test_x, test_y = svhn.load_numpy(
        True, binarize_y=True)  # norb.load_resized(size, binarize_y=True)
    ...
    if True:
        ...
        model = GPUVAE_YZ_X(None, n_x, n_y, n_hidden, n_z, n_hidden[::-1], nonlinear,
                            nonlinear, type_px, type_qz=type_qz, type_pz=type_pz, prior_sd=100, init_sd=1e-2)
...
if dataset == 'mnist':
    ...
    if True:
        ...
        def f_dec(x): return x

if True:
    # Some interesting analogies
    if dataset == 'mnist':
        idxs = np.asarray(
            [[7910, 8150, 3623, 2645, 4066, 9660, 5083, 948, 2595, 2]]).T
    elif dataset == 'svhn':
        idxs = np.asarray([[2439, 820, 6590, 24106, 23978, 18466, 191, 20638, 8496,
                            8779, 25783, 3926, 91, 6904, 2865, 9107, 23066, 14359, 24415, 1754]]).T
...
for sample in range(idxs.shape[1]):
    ...
    image = paramgraphics.mat_to_img(
        human_x, dim_input, colorImg=colorImg, tile_shape=tile_shape1)
    ...
    q_mean, q_logvar = model.dist_qz['z'](
        f_enc(human_x).astype(np.float32), human_y, A)
    z = q_mean
    ...
    if draw_rows:
        ...
        _z = np.tile(z, n_y)
    _, _, _z_confab = model.gen_xz(
        {'y': y}, {'z': _z}, n_batch=n_y * n_batch_w)
    x_samples = f_dec(_z_confab['x'])

    image = paramgraphics.mat_to_img(
        x_samples, dim_input, colorImg=colorImg, tile_shape=tile_shape2)
