...
if dataset == 'svhn':
    ...
    if True:
        ...
        def f_dec(x):
            result = pca['eigvec'].dot(
                x * np.sqrt(pca['eigval'])) * pca['x_sd'] + pca['x_center']
...
for i in range(2000):
    ...
    z = np.sqrt(1 - noise_var) * z + np.sqrt(noise_var) * \
        np.random.standard_normal(z.shape)
    ...
    if False:
        image = paramgraphics.mat_to_img(
            x_samples, dim_input, colorImg=colorImg, tile_shape=tile_shape)
        plt.clf()
...
import os
os.system("ffmpeg -start_number 0 -i " + logdir +
          "/%d.png -c:v libx264 -pix_fmt yuv420p -r 30 " + target_fname)
