...
class BNModel(object):
    ...
    def __init__(self, theano_warning='raise', hessian=True):
        ...
        self.allvars = w.values() + x.values() + z.values() + \
        ...
        self.f_dlogpxz_dwz = theanofunction(
            self.allvars, [logpx, logpz] + dlogpxz_dwz)
        ...
        if False:
            ...
            dlogpxmc_dw = T.grad(logpxmc, w.values(),
                                 disconnected_inputs=theano_warning)
            self.f_dlogpxmc_dw = theanofunction(
                self.allvars, [logpxmc] + dlogpxmc_dw)
        ...
        if True and len(z) > 0:
            ...
            self.f_dfd_dw = theanofunction(
                self.allvars + gz2, [logpx, logpz, fd] + dfd_dw)
    ...
    def dlogpxz_dwz(self, w, x, z):
        ...
        logpx, logpz, gw, gz = r[0], r[1], dict(
            zip(w.keys(), r[2:2 + len(w)])), dict(zip(z.keys(), r[2 + len(w):]))
    ...
    def dfd_dw(self, w, x, z, gz2):
        ...
        A = self.get_A(x)
        r = self.f_dfd_dw(*(w.values() + x.values() +
                            z.values() + [A] + gz2.values()))
        logpx, logpz, fd, gw = r[0], r[1], r[2], dict(
            zip(w.keys(), r[3:3 + len(w)]))
