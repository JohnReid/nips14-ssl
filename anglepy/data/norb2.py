...
def load_numpy_subclasses(size=48, binarize_y=False):
    ...
    # computes which of the 50 subclasses
    train_y = (train_cat[:] * n_ipc + train_info[:, 0]).reshape(1, n)
    ...
    # computes which of the 50 subclasses
    x = np.hstack((train_x_left, train_x_right, test_x_left, test_x_right))
...
def convert_orig_to_np():
    ...
    files = (('train_cat', train_cat), ('train_dat_96', train_dat), ('train_info', train_info),
             ('test_cat', test_cat), ('test_dat_96', test_dat), ('test_info', test_info))
