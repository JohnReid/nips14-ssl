...
def load_theano(size=28):
    ...
    def shared_dataset(data_xy, borrow=True):
        ...
        shared_x = theano.shared(np.transpose(np.asarray(
            data_x, dtype=theano.config.floatX)), borrow=borrow)
        shared_y = theano.shared(np.transpose(np.asarray(
...
def save_reshaped(shape):
    def reshape_digits(x, shape):
        ...
        for i in range(nrows):
            result[i, :] = rebin(x[i, :].reshape(
                (28, 28)), shape).reshape((1, ncols))
        return result
    ...
    f.close()
    f = gzip.open(os.path.dirname(__file__) + '/mnist_' +
                  str(shape[0]) + '_.pkl.gz', 'wb')
