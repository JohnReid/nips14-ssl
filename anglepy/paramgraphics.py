...
def tile_raster_images(X, img_shape, tile_shape, tile_spacing=(0, 0),
    ...
    if colorImg:
        ...
        X = (X[:, 0:channelSize], X[:, channelSize:2 * channelSize],
             X[:, 2 * channelSize:3 * channelSize], None)
        ...
        for i in xrange(4):
            if X[i] is None:
                ...
                out_array[:, :, i] = tile_raster_images(
                    xi, img_shape, tile_shape, tile_spacing, False, output_pixel_vals)
...
def mat_to_img(w, dim_input, scale=False, colorImg=False, tile_spacing=(1, 1), tile_shape=0):
    ...
    imgs = tile_raster_images(X=w.T, img_shape=dim_input, tile_shape=tile_shape,
                              tile_spacing=tile_spacing, scale=scale, colorImg=colorImg)
