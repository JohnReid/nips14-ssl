...
class FuncLikelihood():
    ...
    def val(self, w, z):
        ...
        logpx, logpz = tuple(zip(*[self.subval(i, w, z)
    ...
    def grad(self, w, z):
        ...
        logpxi, logpzi, gwi, _ = tuple(
            zip(*[self.subgrad(i, w, z) for i in range(self.n_minibatches)]))
...
class FuncLikelihoodPar():
    ...
    def subval(self, i, w, z):
        ...
        @interactive
        def ll(w, z, k):
            _x = ndict.getCols(my_x, k * my_n_batch, (k + 1)
                               * my_n_batch)  # @UndefinedVariable
            if z == None:
    ...
    def subgrad(self, i, w, z):
        ...
        def dlogpxz_dwz(w, z, k):
            _x = ndict.getCols(my_x, k * my_n_batch, (k + 1)
                               * my_n_batch).copy()  # @UndefinedVariable
            if z == None:
        ...
        for j in range(len(self.c)):
            ...
            if _z != None:
                _z = ndict.getCols(z, j * self.n_batch, (j + 1) * self.n_batch)
            tasks.append(self.c.load_balanced_view(
            ).apply_async(dlogpxz_dwz, w, _z, i))
...
class FuncLikelihoodMC():
    ...
    def grad(self, w):
        logpxi, gwi = tuple(zip(*[self.subgrad(i, w)
                                  for i in range(self.n_minibatches)]))
