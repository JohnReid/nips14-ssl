...
class VAEModel(object):
    ...
    def __init__(self, theano_warning='raise'):
        ...
        allvars = v.values() + w.values() + x.values() + z.values() + \
        ...
        weights = T.dmatrix()
        dL_weighted_dw = T.grad(
            (weights * (logpx + logpz - logqz)).sum(), v.values() + w.values())
        self.f_dL_weighted_dw = theanofunction(
            allvars + [weights], [logpx + logpz - logqz, weights * (logpx + logpz - logqz)] + dL_weighted_dw)

        # prior
        dlogpw_dw = T.grad(logpv + logpw, v.values() +
                           w.values(), disconnected_inputs='ignore')
        self.f_logpw = theanofunction(v.values() + w.values(), [logpv, logpw])
        self.f_dlogpw_dw = theanofunction(
            v.values() + w.values(), [logpv, logpw] + dlogpw_dw)
    ...
    def dL_weighted_dw(self, v, w, x, z, weights):
        ...
        L_unweighted, L_weighted, gv, gw = r[0], r[1], dict(zip(
            v.keys(), r[2:2 + len(v)])), dict(zip(w.keys(), r[2 + len(v):2 + len(v) + len(w)]))
        self.checknan(v, w, gv, gw)
        gv, gw = self.gw_to_numpy(gv, gw)
        ...
        for i in x:
            if (x[i].shape[1] != 1):
                raise Exception("{} {} {} ".format(
                    x[i].shape[0], x[i].shape[1], n_batch))
