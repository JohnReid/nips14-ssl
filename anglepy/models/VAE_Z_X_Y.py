...
class VAE_Z_X_Y(ap.VAEModel):
    ...
    def factors(self, v, w, x, z, A):
        ...
        self.dist_qz['z'] = theanofunc(
            [x['x']] + v.values() + [A], [q_mean, q_logvar])
        ...
        if self.type_px == 'bernoulli':
            ...
            self.dist_px['x'] = theanofunc(
                [_z] + w.values() + [A], [x_mean, x_logvar])
        else:
            raise Exception("")

        # Note: logpx is a row vector (one element per sample)
        # logpx = logp(x|z,w) + logp(y|x,w)
        logpx = T.dot(np.ones((1, self.n_x)), _logpx)

        # log p(y|x)
        _logpy = T.dot(w['out_w_y'], hidden_q[-1]) + T.dot(w['out_b_y'], A)
        py = T.nnet.softmax(_logpy.T).T
        logpy = (- T.nnet.categorical_crossentropy(py.T,
                                                   x['y'].T).T).reshape((1, -1))
        self.dist_px['y'] = theanofunc(
            [x['x']] + v.values() + w.values() + [A], py)
        ...
        dL2_dw = T.grad((logpx + logpz - logqz).sum(),
                        v.values() + w.values(), disconnected_inputs='ignore')
        allvars = self.var_v.values() + self.var_w.values() + \
            [x['x']] + z.values() + [A]
    ...
    def dL2_dw(self, v, w, x, z):
        ...
        logpx, logpz, logqz, gv, gw = r[0], r[1], r[2], dict(zip(
            v.keys(), r[3:3 + len(v)])), dict(zip(w.keys(), r[3 + len(v):3 + len(v) + len(w)]))
    ...
    def gen_xz(self, v, w, x, z, n_batch):
        ...
        if x.has_key('x') and not z.has_key('z'):
            ...
            q_mean, q_logvar = self.dist_qz['z'](
                *([x['x']] + v.values() + [A]))
            _z['mean'] = q_mean
            _z['logvar'] = q_logvar
        ...
        if self.type_px == 'bernoulli':
            ...
            x_mean, x_logvar = self.dist_px['x'](
                *([z['z']] + w.values() + [A]))
            _z['x'] = x_mean
            if not x.has_key('x'):
