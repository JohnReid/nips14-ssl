...
def main(n_z, n_hidden, dataset, seed, gfx=True, _size=None):
    ...
    import time
    logdir = 'results/gpulearn_yz_x_' + dataset + '_' + \
        str(n_z) + '-' + str(n_hidden) + '-' + str(int(time.time())) + '/'
    ...
    if dataset == 'mnist':
        ...
        x = {'x': train_x[:, :].astype(
            np.float32), 'y': train_y[:, :].astype(np.float32)}
        x_valid = {'x': valid_x.astype(
            np.float32), 'y': valid_y.astype(np.float32)}
        L_valid = 1
        dim_input = (size, size)
        n_x = size * size
        n_y = 10
        ...
        size = _size  # 48
        train_x, train_y, test_x, test_y = norb.load_resized(
            size, binarize_y=True)
        ...
        # Do PCA
        f_enc, f_dec, pca_params = pp.PCA(
            _x['x'][:, :10000], cutoff=2000, toFloat=False)
        ndict.savez(pca_params, logdir + 'pca_params')

        x = {'x': f_enc(train_x).astype(np.float32),
        ...
        if True:
            # Works
            f_enc, f_dec, pca_params = pp.PCA(
                _x['x'][:, :10000], cutoff=600, global_sd=True, toFloat=True)
            ndict.savez(pca_params, logdir + 'pca_params')
        elif False:
            # Doesn't work
            f_enc, f_dec, pp_params = pp.normalize_noise(
                _x['x'][:, :50000], noise_sd=0.01, global_sd=True, toFloat=True)
        else:
            # Doesn't work
            f_enc, f_dec, params = pp.normalize_random(
                x=x[:, :10000], global_sd=True, toFloat=True)
            ndict.savez(params, logdir + 'normalize_random_params')

        n_valid = 5000
        x = {'x': f_enc(_x['x'][:, :-n_valid]).astype(np.float32),
             'y': _x['y'][:, :-n_valid].astype(np.float32)}
        x_valid = {'x': f_enc(_x['x'][:, :n_valid]).astype(
            np.float32), 'y': _x['y'][:, :n_valid].astype(np.float32)}
    ...
    updates = get_adam_optimizer(
        alpha=3e-4, beta1=0.9, beta2=0.999, weight_decay=0)
    model = GPUVAE_YZ_X(updates, n_x, n_y, n_hidden_q, n_z, n_hidden_p[::-1], 'softplus', 'softplus',
                        type_px=type_px, type_qz='gaussianmarg', type_pz='gaussianmarg', prior_sd=1, uniform_y=True)
    ...
    z_sample = {'z': np.repeat(np.random.standard_normal(
        size=(n_z, 12)), 12, axis=1).astype(np.float32)}
    y_sample = {'y': np.tile(np.random.multinomial(
        1, [1. / n_y] * n_y, size=12).T, (1, 12))}
    ...
    def hook(epoch, t, ll):
        ...
        ll_valid, _ = model.est_loglik(
            x_valid, n_samples=L_valid, n_batch=n_batch, byteToFloat=byteToFloat)
        ...
        if ll_valid > ll_valid_stats[0]:
            ...
            ll_valid_stats[1] += 1
            # Stop when not improving validation set performance in 100
            # iterations
        ...
        if gfx:
            ...
            image = paramgraphics.mat_to_img(
                f_dec(v['w0x'][:].T), dim_input, True, colorImg=colorImg)
            image.save(logdir + 'q_w0x' + tail, 'PNG')

            image = paramgraphics.mat_to_img(
                f_dec(w['out_w'][:]), dim_input, True, colorImg=colorImg)
            ...
            image = paramgraphics.mat_to_img(
                f_dec(_z_confab['x']), dim_input, colorImg=colorImg)
            ...
            if n_z == 2:
                ...
                mosaic = Image.new(
                    "RGB", (submosaic_width * mosaic_w, submosaic_offset + submosaic_height * mosaic_h))
                ...
                for digit in range(0, n_y):
                    ...
                    for i in range(0, n_width):
                        for j in range(0, n_width):
                            _z['z'][0, n_width * i +
                                    j] = scipy.stats.norm.ppf(float(i) / n_width + 0.5 / n_width)
                            _z['z'][1, n_width * i +
                                    j] = scipy.stats.norm.ppf(float(j) / n_width + 0.5 / n_width)

                    _x, _, _z_confab = model.gen_xz(
                        _x, _z, n_batch=n_batch_plot)
                    x_samples = _z_confab['x']
                    image = paramgraphics.mat_to_img(
                        f_dec(x_samples), dim_input, colorImg=colorImg, tile_spacing=(0, 0))

                    #image.save(logdir+'samples_digit_'+str(digit)+'_'+tail, 'PNG')
                    mosaic_x = (digit % mosaic_w) * submosaic_width
                    mosaic_y = submosaic_offset + \
                        int(digit / mosaic_w) * submosaic_height
                    mosaic.paste(image, (mosaic_x, mosaic_y))

                draw = ImageDraw.Draw(mosaic)
                draw.text((1, 1), "Epoch #" + str(epoch) +
                          " Loss=" + str(int(ll)))
    ...
    # Optimize
    dostep = epoch_vae_adam(model, x, n_batch=n_batch,
                            bernoulli_x=bernoulli_x, byteToFloat=byteToFloat)
    loop_va(dostep, hook)
