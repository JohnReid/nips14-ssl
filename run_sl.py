...
if dataset == 'mnist':
    ...
    _, train_y, _, _, test_x, test_y = mnist.load_numpy(
        size=28, binarize_y=False)
    ...
    n_hidden = 500, 500
    updates = None
    model = GPUVAE_YZ_X(updates, n_x, n_y, n_hidden, n_z, n_hidden, 'softplus', 'softplus',
                        type_px='bernoulli', type_qz='gaussianmarg', type_pz='gaussianmarg', prior_sd=1, uniform_y=True)
...
def get_lowerbound():
    ...
    for _class in range(n_y):
        ...
        y[_class, :] = 1
        _lb = model.eval({'x': test_x.astype(np.float32),
