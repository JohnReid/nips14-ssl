...
def main(n_z, n_hidden, dataset, seed, comment, gfx=True):
    ...
    # Initialize logdir
    import time
    logdir = 'results/gpulearn_z_x_' + dataset + '_' + \
        str(n_z) + '-' + str(n_hidden) + '_' + \
        comment + '_' + str(int(time.time())) + '/'
    ...
    if dataset == 'mnist_binarized':
        ...
        f_enc, f_dec, pca_params = pp.PCA(
            x['x'][:, :10000], cutoff=600, toFloat=True)
    ...
    model = GPUVAE_Z_X(updates, n_x, n_hidden, n_z, n_hidden[::-1], nonlinear, nonlinear,
                       type_px, type_qz=type_qz, type_pz=type_pz, prior_sd=100, init_sd=1e-3)
            ...
            if 'pca' not in dataset and 'random' not in dataset and 'normalized' not in dataset:
                ...
                if 'w0' in v:
                    image = paramgraphics.mat_to_img(
                        f_dec(v['w0'][:].T), dim_input, True, colorImg=colorImg)
                ...
                if 'out_unif' in w:
                    image = paramgraphics.mat_to_img(
                        f_dec(w['out_unif'].reshape((-1, 1))), dim_input, True, colorImg=colorImg)
                ...
                if n_z == 2:
                    ...
                    image = paramgraphics.mat_to_img(
                        f_dec(x_samples), dim_input, colorImg=colorImg)
                ...
                image = paramgraphics.mat_to_img(
                    x_samples, dim_input, colorImg=colorImg)
                image.save(logdir + 'samples' + tail, 'PNG')
